...
//
//    getEventsReturns, err := UnmarshalGetEventsReturns(bytes)
//    bytes, err = getEventsReturns.Marshal()

package getEvents

import "encoding/json"

func UnmarshalGetEventsReturns(data []byte) (GetEventsReturns, error) {
	var r GetEventsReturns
	err := json.Unmarshal(data, &r)
	return r, err
}

func (r *GetEventsReturns) Marshal() ([]byte, error) {
	return json.Marshal(r)
}

type GetEventsReturns struct {
...
type Event struct {
	...
	Attachments             []EventAttachment   `json:"attachments"`                       // File attachments for the event. Currently only Google Drive attachments are supported.; In order to modify attachments the supportsAttachments request parameter should be set to; true.; There can be at most 25 attachments per event,
	Attendees               []EventAttendee     `json:"attendees"`                         // The attendees of the event. See the Events with attendees guide for more information on; scheduling events with other calendar users.
...
type ConferenceData struct {
	...
	EntryPoints        []EntryPoint             `json:"entryPoints"`                  // Information about individual conference entry points, such as URLs or phone numbers.; All of them must belong to the same conference.; Either conferenceSolution and at least one entryPoint, or createRequest is required.
...
type EntryPoint struct {
	...
	EntryPointFeatures []string `json:"entryPointFeatures"`       // Features of the entry point, such as being toll or toll-free. One entry point can have; multiple features. However, toll and toll-free cannot be both set on the same entry point.
...
type Reminders struct {
	Overrides  []EventReminder `json:"overrides"`            // If the event doesn't use the default reminders, this lists the reminders specific to the; event, or, if not set, indicates that no reminders are set for this event. The maximum; number of override reminders is 5.
