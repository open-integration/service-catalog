...
import (
	...
	"github.com/open-integration/service-catalog/exec/pkg/endpoints/command"
)
...
func (s *Service) Call(context context.Context, req *api.CallRequest) (*api.CallResponse, error) {
	...
	case "command":
		args, err := command.UnmarshalCommandArguments([]byte(req.Arguments))
		if resp := buildErrorResponse(err); resp != nil {
			return resp, nil
		}
		opt := command.CommandOptions{
			Context:   context,
			LoggerFD:  req.Fd,
			Arguments: &args,
		}
		res, err := command.Command(opt)
		if resp := buildErrorResponse(err); resp != nil {
			return resp, nil
		}
