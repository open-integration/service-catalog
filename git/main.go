...
import (
	...
	"github.com/open-integration/service-catalog/git/pkg/endpoints/clone"
...
func (s *Service) Call(context context.Context, req *api.CallRequest) (*api.CallResponse, error) {
	...
	case "clone":
		args, err := clone.UnmarshalCloneArguments([]byte(req.Arguments))
		if resp := buildErrorResponse(err); resp != nil {
			return resp, nil
		}
		opt := clone.CloneOptions{
			Context:   context,
			LoggerFD:  req.Fd,
			Arguments: &args,
		}
		res, err := clone.Clone(opt)
		if resp := buildErrorResponse(err); resp != nil {
			return resp, nil
		}
